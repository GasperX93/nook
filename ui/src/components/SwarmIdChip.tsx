...
/**
 * Header identity chip (spike/swarm-id-only) — replaces the global wallet
...
export default function SwarmIdChip() {
  const { signer, deriveViaSwarmId, deriving, clear, isSwarmIdIdentity } = useDerivedKey()
  ...
  if (!signer) {
    return (
      <button
        onClick={async () => deriveViaSwarmId()}
        disabled={deriving}
