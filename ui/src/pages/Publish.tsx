...
type PublishType = 'file' | 'folder' | 'website'
type Step = 'select' | 'storage' | 'feed' | 'publishing' | 'done'
        ...
        <div key={label} className="flex items-center gap-2">
          <div className="flex items-center gap-1.5">
            <div
              ...
              style={{ backgroundColor: i <= idx ? 'rgb(var(--accent))' : 'rgb(var(--border))' }}
            />
            <span
              className="text-[10px] uppercase tracking-widest"
              style={{ color: i <= idx ? 'rgb(var(--fg-muted))' : 'rgb(var(--border))' }}
            >
              {label}
            </span>
          </div>
          {i < 2 && <ChevronRight size={10} style={{ color: 'rgb(var(--border))' }} />}
        </div>
      ))}
    </div>
  )
  ...
  return (
    <button
      ...
      style={{
        ...
        backgroundColor: active ? 'rgba(247,104,8,0.08)' : 'transparent',
...
function TopUpModal({ stamp, onClose }: { stamp: Stamp; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <p className="text-sm font-semibold">Top up stamp</p>
          <p className="text-xs mt-1 font-mono" style={{ color: 'rgb(var(--fg-muted))' }}>
            {stamp.label || `${stamp.batchID.slice(0, 20)}…`}
          </p>
          <p className="text-xs mt-0.5" style={{ color: 'rgb(var(--fg-muted))' }}>
            {depthToCapacity(stamp.depth)} · {ttlToDays(stamp.batchTTL)} left
          </p>
        </div>
...
export default function Publish() {
  ...
  const [publishType, setPublishType] = useState<PublishType>('file')
  const [content, setContent] = useState<SelectedContent | null>(null)
  const [dragging, setDragging] = useState(false)
  const [plan, setPlan] = useState<StoragePlan>({ sizeIdx: 1, durationIdx: 1 })
  const [stampMode, setStampMode] = useState<'existing' | 'new'>('existing')
  ...
  function acceptContent(c: SelectedContent) {
    setContent(c)
    // Auto-select the cheapest tier that fits the content size
    const idx = SIZE_PRESETS.findIndex(s => (1 << s.depth) * 4096 >= c.size)
    setPlan(p => ({ ...p, sizeIdx: idx === -1 ? SIZE_PRESETS.length - 1 : idx }))
    setStep('storage')
  }
  ...
  function openPicker() {
    if (publishType === 'file') fileInputRef.current?.click()
  ...
  async function publish() {
    ...
    try {
      ...
      const expiresAt = existingStamp
        ? Date.now() + existingStamp.batchTTL * 1000
        : Date.now() + selectedDuration.months * 30 * 24 * 60 * 60 * 1000
      addRecord({
      ...
      setResult({ hash: reference, expiresAt, feedManifestAddress })
      setStep('done')
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {step !== 'done' && step !== 'publishing' && <StepDots step={step} />}

      {/* ── Step 1: Select ── */}
      {step === 'select' && (
        <div className="space-y-4">
          {/* Welcome banner for new users */}
          {!beeChecking && beeOnline && bzzBalance === 0 && usableStamps.length === 0 && (
            <div
          ...
          {/* Drop zone */}
          <div
            onClick={beeOffline ? undefined : openPicker}
          ...
          >
            {beeOffline ? (
              <div className="flex flex-col items-center gap-2 py-12 px-6 text-center">
                ...
                <p className="text-xs" style={{ color: 'rgb(var(--fg-muted))' }}>
                  Start your node to publish content
                </p>
              </div>
            ) : (
      ...
      {step === 'storage' && content && (
        <div className="space-y-6">
          ...
          {/* Mode toggle */}
          <div className="flex gap-1 p-1 rounded-lg" style={{ backgroundColor: 'rgb(var(--bg))' }}>
            {(['existing', 'new'] as const).map(mode => (
              <button
                ...
                style={{
                  backgroundColor: stampMode === mode ? 'rgb(var(--bg-surface))' : 'transparent',
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="text-sm font-medium">Make this content updatable</p>
                <p className="text-xs mt-0.5" style={{ color: 'rgb(var(--fg-muted))' }}>
                  Get a fixed address you can point to new content later
                </p>
              </div>
              <button
      ...
      {step === 'publishing' && (
        <div className="flex flex-col items-center gap-4 py-16">
          ...
          <div className="text-center space-y-3 w-full max-w-xs">
            ...
            <p className="text-xs" style={{ color: 'rgb(var(--fg-muted))' }}>
              {content?.type === 'file' ? content.name : `${content?.entries.length} files`}
            </p>
            {uploadProgress !== null && (
